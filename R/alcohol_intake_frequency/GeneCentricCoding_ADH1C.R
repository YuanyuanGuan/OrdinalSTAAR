...
load("/mnt/project/UKB_500K_WGS_staarpipeline/Multiclass/alcohol_intake_frequency/WGS_alcohol_intake_frequency_fullDat_with_PRS_PCs.20250619.Rdata")
...
#
#   cat("\n--- Debugging format_results ---\n")
#   print("Structure of p_matrix:")
#   print(str(p_matrix))
#   print("Row names of p_matrix:")
#   print(rownames(p_matrix))
#   print("Column names of p_matrix:")
#   print(colnames(p_matrix))
#   cat("---------------------------------\n\n")
#
...
format_results <- function(results_list) {
  ...
  # --- Part 1: Handle cases with no valid results ---
  # 增加对 gene_info 的检查
  if (is.null(results_list) || is.na(results_list$OrdinalSTAAR_O) || is.null(results_list$gene_info)) {
    return(NULL)
  }

  # --- Part 2: Extract components and perform robust checks ---
  gene_info <- results_list$gene_info
  p_matrix <- results_list$OrdinalSTAAR_pvalue

  # 确保 p_matrix 是一个有正确维度的矩阵
  if (!is.matrix(p_matrix) || nrow(p_matrix) < 6 || ncol(p_matrix) < 2) {
    warning("p_matrix has unexpected dimensions. Skipping formatting.")
    return(NULL)
  }

  # --- Part 3: Create the single-row data.frame (安全提取版) ---

  # 使用一个辅助函数来安全地提取值，如果不存在则返回 NA
  ...
  output_df <- data.frame(
    ...
    # P-values (使用 @ 运算符安全索引，如果名称不存在会返回 NA)
    `SKAT(1,25)` = p_matrix["SKAT-(1,25)", "Beta"],
    `SKAT(1,1)` = p_matrix["SKAT-(1,1)", "Beta"],
    `Burden(1,25)` = p_matrix["Burden-(1,25)", "Beta"],
    `Burden(1,1)` = p_matrix["Burden-(1,1)", "Beta"],
    ...
    # Overall omnibus p-values
    `ACAT-O` = safe_extract(results_list, "ACAT_O"),
    `STAAR-O` = safe_extract(results_list, "OrdinalSTAAR_O"),
...
for (current_chr in unique_chromosomes) {
  ...
  for (i in 1:nrow(tasks_for_this_chr)) {
    ...
    if (!is.null(analysis_results_list_obj) && !is.na(analysis_results_list_obj$OrdinalSTAAR_O[1])) {
      ...
      # message("  -> Formatting results for CSV summary...")
      ...
      message(paste0("  -> No valid results for '", gene_name, "' category '", categories, "'. Skipping."))
    }
  }
