...
plof <- function(gene_name, genofile, objNull, genes_info, variant_type = NULL,
  ...
  mask <- (GENCODE.EXONIC %in% c("stopgain", "stoploss")) | 
    (GENCODE %in% c("splicing", "exonic;splicing", "ncRNA_splicing", "ncRNA_exonic;splicing"))
  ...
  # Need to subset annotation to the masked variants
