...
ptv <- function(gene_name, genofile, objNull, genes_info, variant_type = NULL,
  ...
  if (variant_type == "SNV") {
    mask <- (GENCODE.EXONIC %in% c("stopgain", "stoploss")) | (GENCODE %in% c("splicing", "exonic;splicing"))
  } else if (variant_type == "Indel") {
    mask <- GENCODE.EXONIC %in% c("frameshift deletion", "frameshift insertion")
  } else { # variant
    mask <- (GENCODE.EXONIC %in% c("stopgain", "stoploss", "frameshift deletion", "frameshift insertion")) | (GENCODE %in% c("splicing", "exonic;splicing"))
  }
