ptv <- function(gene_name, genofile, objNull, genes_info, variant_type = NULL,
  ...
  ## ptv: SNV = stopgain/stoploss + splicing/exonic;splicing
  ##      Indel = frameshift deletion/insertion
  ...
  if (variant_type == "SNV") {
    lof.in.ptv <- (GENCODE.EXONIC.Category %in% c("stopgain","stoploss")) |
      (GENCODE.Category %in% c("splicing","exonic;splicing"))
  }
  if (variant_type == "Indel") {
    lof.in.ptv <- (GENCODE.EXONIC.Category %in% c("frameshift deletion","frameshift insertion"))
  }
  if (variant_type == "variant") {
    lof.in.ptv.snv   <- (GENCODE.EXONIC.Category %in% c("stopgain","stoploss")) |
      (GENCODE.Category %in% c("splicing","exonic;splicing"))
    lof.in.ptv.indel <- (GENCODE.EXONIC.Category %in% c("frameshift deletion","frameshift insertion"))
    lof.in.ptv <- lof.in.ptv.snv | lof.in.ptv.indel
  }
  variant.id.gene <- variant.id.gene[lof.in.ptv]
