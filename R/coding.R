coding <- function(gene_name, genofile, objNull, genes_info, variant_type = NULL,
  ...
  lof.in.coding <- (GENCODE.EXONIC.Category=="stopgain")|(GENCODE.EXONIC.Category=="stoploss")|
  ...
  variant.id.gene.category <- variant.id.gene[lof.in.plof_ds]
  ...
  if (length(variant.id.gene.category) < 2) {
    ...
    ## Annotation
    Anno.Int.PHRED.sub.category <- Anno.Int.PHRED.sub[lof.in.plof_ds, , drop = FALSE]
    ...
    Anno.Int.PHRED.sub.category <- Anno.Int.PHRED.sub.category[getGeno$include_index, , drop = FALSE]
    
    if (variant_type == "SNV" && ncol(Anno.Int.PHRED.sub.category) >= 2 &&
        anyNA(Anno.Int.PHRED.sub.category[,2])) {
      ...
      Anno.Int.PHRED.sub.category <- Anno.Int.PHRED.sub.category[include_index, , drop = FALSE]
    }
    ...
    if (is.null(dim(Geno)) || ncol(Geno) == 0) {
      ...
      result.plof_ds <- try(
        OrdinalSTAAR(Geno, MAF, MAC, objNull, annotation_phred = Anno.Int.PHRED.sub.category,
  ...
  lof.in.plof <- (GENCODE.EXONIC.Category %in% c("stopgain","stoploss")) |
    (GENCODE.Category %in% c("splicing","exonic;splicing","ncRNA_splicing","ncRNA_exonic;splicing"))
  ...
  if (length(variant.id.gene.category) < 2) {
    ...
    ## Annotation
    Anno.Int.PHRED.sub.category <- Anno.Int.PHRED.sub[lof.in.plof, , drop = FALSE]
  ...
  if (length(variant.id.gene.category) < 2) {
    ...
    ## Annotation
    Anno.Int.PHRED.sub.category <- Anno.Int.PHRED.sub[lof.in.synonymous, , drop = FALSE]
  ...
  if (length(variant.id.gene.category) < 2) {
    ...
    ## Annotation
    Anno.Int.PHRED.sub.category <- Anno.Int.PHRED.sub[lof.in.missense, , drop = FALSE]
  ...
  if (length(variant.id.gene.category) < 2) {
    ...
    ## Annotation
    Anno.Int.PHRED.sub.category <- Anno.Int.PHRED.sub[lof.in.dmissense, , drop = FALSE]
  ...
  ## merge disruptive into missense (CCT) ######
  if (!is.na(result.missense$OrdinalSTAAR_O) & !is.na(result.dmissense$OrdinalSTAAR_O)) {
    ...
    result.missense$OrdinalSTAAR_pvalue <- cbind(result.missense$OrdinalSTAAR_pvalue,
                                                 result.dmissense$OrdinalSTAAR_pvalue[, 1])
  ...
  OrdinalSTAAR_O_all <- c("plof"       = result.plof$OrdinalSTAAR_O,
                          "plof_ds"    = result.plof_ds$OrdinalSTAAR_O,
                          "missense"   = result.missense$OrdinalSTAAR_O,
                          "dmissense"  = result.dmissense$OrdinalSTAAR_O,
